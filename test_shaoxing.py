...
def main(args, config):
    ...
    print("test set number：", len(X_val))
    X_train, y_train = down_sample_train_data(X_train, y_train, config['finetune_sample_rate'])
    train_dataset = FinetuneDataset(X_train, y_train, "shaoxing")
    train_dataloader = DataLoader(train_dataset,
                                  ...
                                  drop_last=True,
                                  collate_fn=None)
    test_dataset = FinetuneDataset(X_test, y_test, "shaoxing")
    test_dataloader = DataLoader(test_dataset,
                                 ...
                                 drop_last=True,
                                 collate_fn=None)
    val_dataset = FinetuneDataset(X_val, y_val, "shaoxing")
    ...
    if config["ecg_model_name"] == 'resnet':
        ...
        ecg_model = ResNet1D(in_channels=1, base_filters=768, kernel_size=1, stride=2, groups=config["test_batch_size"],
        ...
        ecg_model = resnet1d_wang(num_classes=config['test_shaoxing_class_nums'], input_channels=12, kernel_size=5,
    ...
    if config['finetune']:
        ...
        for epoch in range(start_epoch, max_epoch):
            ...
            if best_val_auc < val_auc:
                ...
                file_path = "/home/user/tyy/project/ked/trained_model/checkpoints_finetune/finetune_shaoxing_" + str(
...
def test(model, ecg_encoder, text_encoder, tokenizer, data_loader, device, args, config):
    ...
    f = open('/home/tyy/project/ecgfm_ked/dataset/shaoxing/mlb.pkl', 'rb')
    data = pickle.load(f)
    label_list = [all_label_map[item] for item in data.classes_]

    save_result_path = os.path.join(args.output_dir, config["result_shaoxing_save_name"])
    dist_csv_col = ['metric', "Atrial Flutter", "Atrial Fibrillation","Atrial Tachycardia",
                           "Sinus Irregularity", "Sinus Bradycardia","Sinus Rhythm","Sinus Tachycardia","Supraventricular Tachycardia", 'mean']
    ...
    if not config["use_ecgNet_Diagnosis"] == "ecgNet":
        num_class = config['test_shaoxing_class_nums']
...
if __name__ == '__main__':
    ...
    for model in model_list:
        ...
        parser.add_argument('--output_dir', default='./output_test/shaoxing')
        parser.add_argument('--finetune_output_dir', default='./output/output_finetune/shaoxing')
        ...
        parser.add_argument('--result_save_name', default='result_8_label_augment_new.csv')

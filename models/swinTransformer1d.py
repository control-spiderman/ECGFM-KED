...
class SwinTransformer1D(nn.Module):
    ...
    def __init__(self,
        ...
        self.window_size = window_size
        self.patch_size = patch_size
        self.lstm1 = nn.LSTM(input_size=768, hidden_size=384, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):   # (64,12,1000)
        """Forward function."""
        x = self.patch_embed(x)   # (64,128,250)

        x = self.pos_drop(x)
        # print(x.shape)

        for layer in self.layers:
            x = layer(x.contiguous())   # (64,256,125), (64,512,63), (64,1024,32), (64,1024,32)

        # print(x.shape)

        x = rearrange(x, 'n c l -> n l c')  # (64,32,768)
        lstm_output, (final_hidden_state, final_cell_state) = self.lstm1(x)  # (8,19,256)
        x = self.norm(lstm_output)
        x = rearrange(x, 'n l c -> n c l')  # (64,768,32)
        # lstm_output, (final_hidden_state, final_cell_state) = self.lstm1(x)  # (8,19,256)
...
if __name__ == "__main__":
    net = SwinTransformer1D(
        ...
        patch_norm=False,
        use_checkpoint=False)
    x = torch.rand(64, 12, 1000) #2, 32, 2000
    print(net(x).shape)